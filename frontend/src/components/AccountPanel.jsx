...
import { CircleUserRound, LogOut, MailCheck, Trash2, X } from "lucide-react";

export default function AccountPanel({ open, account, loading, onClose, onSignOut, onDelete }) {
    ...
    <div className="fixed inset-0 z-[110] flex items-end justify-center bg-black/70 p-0 backdrop-blur-sm sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="account-panel-title">
      <div className="max-h-[92dvh] w-full max-w-xl overflow-y-auto rounded-t-lg border border-border-default bg-bg-secondary shadow-2xl sm:rounded-lg">
        ...
        <div className="space-y-4 px-5 py-5">
          ...
          <div><h3 className="text-sm font-semibold text-text-primary">最近积分流水</h3><div className="mt-2 divide-y divide-border-subtle rounded-lg border border-border-subtle bg-bg-tertiary">{(account?.transactions || []).slice(0, 6).map((item) => <div key={item.id} className="flex items-center justify-between gap-3 px-3 py-3 text-sm"><span className="text-text-secondary">{item.type === "signup_bonus" ? "注册奖励" : item.type === "refund" ? "生成退款" : item.type === "spend" ? "生图消费" : item.type === "reserve" ? "生成预扣" : "积分调整"}</span><strong className={Number(item.delta) >= 0 ? "text-mint" : "text-text-primary"}>{Number(item.delta) > 0 ? "+" : ""}{item.delta}</strong></div>)}{loading ? <p className="animate-pulse px-3 py-4 text-sm text-text-muted">正在同步积分流水…</p> : !(account?.transactions || []).length && <p className="px-3 py-4 text-sm text-text-muted">暂时没有流水记录</p>}</div></div>
