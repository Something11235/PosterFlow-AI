...
- 可以使用 tldraw 的选择、绘制、形状、文字、箭头和便签等工具自由排版。
- 在生成画廊中点击单张图片的画布按钮，或勾选多张后点击“加入画布”，即可继续组合设计。
- 可以从电脑批量导入 PNG、JPG、WebP、GIF 或 SVG 图片。
- 新建箭头和文字默认使用醒目的红色与加大尺寸，仍可通过 tldraw 样式工具自行改色。
...
2. 打开“画布 AI”，切换到“标注重绘”，填写本轮修改要求并设置重绘幅度。
...
> 无限画布使用 tldraw SDK。公开生产部署前，请阅读 [tldraw 当前许可证条款](https://tldraw.dev/community/license)，取得适用许可证并配置 `VITE_TLDRAW_LICENSE_KEY`。
...
8. 在“生成参数”中选择预设画幅或填写自定义宽高，并设置画质和生成数量；图生图和迭代修改模式还可以上传参考图或调整参考强度。
...
无限画布的许可证配置与图片服务 API Key 无关。需要配置时，将 `frontend/.env.example` 复制为 `frontend/.env.local`，或在 Vercel 项目的环境变量中增加：

```env
VITE_TLDRAW_LICENSE_KEY=your-tldraw-license-key
```

`VITE_*` 变量会在前端构建时公开，不能用于存放图片服务 API Key。修改 Vercel 环境变量后需要重新部署才会生效。
